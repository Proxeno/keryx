...
/// <summary>Counters describing one inbound stream's ULPFEC recovery.</summary>
/// <param name="MediaPacketsObserved">Media packets handed to the receiver, recovered ones excluded.</param>
/// <param name="FecPacketsObserved">ULPFEC packets handed to the receiver.</param>
/// <param name="PacketsRecovered">Media packets rebuilt from a FEC packet and its survivors.</param>
public readonly record struct UlpFecStats(long MediaPacketsObserved, long FecPacketsObserved, long PacketsRecovered);

/// <summary>
/// Recovers a single lost media packet in an RFC 5109 protection group from the packets that did
/// arrive and the group's ULPFEC packet.
/// </summary>
/// <remarks>
/// <para>
/// The receiver keeps a bounded ring of recently seen media packets and a short list of FEC packets it
/// cannot yet resolve. Each media packet (fed to <see cref="OnMediaPacket"/>) and each FEC packet (fed
/// to <see cref="OnFecPacket"/>) may complete a group that was missing exactly one packet; when it
/// does, the receiver rebuilds the missing packet and queues it, so the caller drains it with
/// <see cref="TryDequeueRecovered"/> and delivers it into the same receive path a retransmission would.
/// A group missing two or more packets is left alone — level-0 ULPFEC repairs one loss per group — and
/// a packet already seen (including one this receiver recovered) never triggers a second recovery, so
/// the path cooperates with NACK/RTX and the jitter buffer instead of double-delivering.
/// </para>
/// <para>
...
public sealed class UlpFecReceiver
{
    ...
    public UlpFecStats GetStats() => new(_mediaObserved, _fecObserved, _recoveredCount);
    ...
    /// <summary>Accepts a decoded ULPFEC payload and attempts to recover a missing packet in its group.</summary>
    /// <param name="fecPayload">The payload of a ULPFEC RTP packet, after any RED unwrapping.</param>
    ...
    {
        if (!UlpFecPacket.TryParse(fecPayload, out var header))
        {
            // Malformed FEC that slipped past the arrival check: discard it.
            return true;
        }
        ...
        for (var bit = 0; bit < UlpFecPacket.ShortMaskLength; bit++)
        {
            var sequenceNumber = UlpFecPacket.SequenceNumberAt(header.SequenceNumberBase, bit);
        ...
        // More than one loss is beyond level-0 ULPFEC; keep the FEC in case a survivor still arrives (a
        // reordered packet or a retransmission) and drops the count to one.
        if (missingCount > 1)
        {
            return false;
        }
    ...
    private bool TryRecover(UlpFecPacket.Header header, ushort missingSequenceNumber)
    {
        const int fixedHeader = UlpFecPacket.FixedRtpHeaderLength;
        ...
        for (var bit = 0; bit < UlpFecPacket.ShortMaskLength; bit++)
        {
            ...
            firstOctet ^= (byte)(survivor[0] & UlpFecPacket.FirstOctetRecoveryMask);
        ...
        // The recovered post-header region cannot be longer than the FEC payload protects; a longer
        // length means the group's protection span could not cover the missing packet.
        if (postHeaderLength > header.ProtectionLength)
        {
            return false;
        }
        ...
        // Second pass folds the survivors' payload octets out of the FEC payload, leaving the missing
        ...
        // Restore the RTP version bits the FEC header displaced with its E/L flags, then stamp the
        // sequence number and the media SSRC the FEC packet never carried.
        recovered[0] = (byte)(UlpFecPacket.RtpVersionBits | (firstOctet & UlpFecPacket.FirstOctetRecoveryMask));
